package com.knoldus
  ...
  class StudentsData {
    ...
    case class Marks(subjectId: Int,studentId: Long, marksObtained: Float) extends Ordered[Marks]{
      def compare(that: Marks) = this.marksObtained compare that.marksObtained
    }
    ...
    def countPassFail(subjectId: Int, percentage: Int, check: String): Int={
      passFail(subjectId, percentage, check.toLowerCase()).size
    }
...
object StudentsData extends App{
  val obj = new StudentsData
  println(obj.countPassFail(2,50,"pass"))
  println(obj.topBottom(5, 5, "bottom"))
}
